...
Connection::Connection(int socket) : _socket(socket), send_pos(0), finish_request(false), index(-1) {
	memset(&_address, 0, sizeof(_address));
}

Connection::Connection(int socket, struct sockaddr_in address)
	: _socket(socket), _address(address), send_pos(0), finish_request(false), index(-1) {}
